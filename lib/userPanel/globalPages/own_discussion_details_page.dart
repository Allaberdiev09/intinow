...
class OwnDiscussionDetailsPage extends StatefulWidget {
  final Map<dynamic, dynamic> discussion;
  final String discussionId;
  OwnDiscussionDetailsPage({Key? key, required this.discussion, required this.discussionId}) : super(key: key);

  @override
  _OwnDiscussionDetailsPageState createState() => _OwnDiscussionDetailsPageState();
}

class _OwnDiscussionDetailsPageState extends State<OwnDiscussionDetailsPage> {
  ...
  Future<void> _deleteDiscussion() async {
    ...
    try {
      DatabaseReference discussionRef = FirebaseDatabase.instance.ref().child('Discussions/${widget.discussionId}');
      await discussionRef.remove();
      Navigator.of(context).pop();
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text("Failed to delete discussion: ${e.toString()}"),
      ));
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.discussion['title'] ?? 'Discussion Details'),
      ),
        ...
        child: Column(
          ...
          children: [
            FutureBuilder<Map<String, dynamic>?>(
              future: getUserData(widget.discussion['createdBy']),
            ...
            Padding(
              ...
              child: Text(
                widget.discussion['title'] ?? 'No Title',
            ...
            Padding(
              padding: const EdgeInsets.all(16.0),
              child: Text(
                widget.discussion['body'] ?? 'No content available',
              ),
            ),
            const Divider(),
            ReplySection(
